...
import {Circle, Rectangle, Renderable} from "@chabb/sprite";

export
function hitTestPoint(point: Point, sprite: Renderable) {
...
export
function hitTestCircle(c1: Circle, c2: Circle, global = false) {
  let vx, vy;
...
export
function multipleCircleCollision(arrayOfCircles: Circle[], global = false) {
  //marble collisions
...
export
function hitTestRectangle(r1: Rectangle, r2: Rectangle, global = false) {
  let hit, vx, vy;
...
export
function hitTestCircleRectangle(c1: Circle, r1: Rectangle, global = false) {
...
export
function hitTestCirclePoint(c1: Circle, point: any, global = false) {
  ...
  point.gx = point.x;
  point.gy = point.y;
  // FIXME ( better type manipulation)
  return hitTestCircle(c1, point as Circle, global);
}
...
function circleRectangleCollision(
    c1: Circle, r1:Rectangle, bounce = false, global = false
) {
...
export
function circlePointCollision(c1: Circle, point: any, bounce = false, global = false) {
  //A point is just a circle with a diameter of
  //1 pixel, so we can cheat. All we need to do is an ordinary circle vs. circle
  //Collision test. Just supply the point with the properties
  //it needs
  ...
  point.gx = point.x;
  point.gy = point.y;
  return circleCollision(c1, point as Circle, bounce, global);
}
/*
bounceOffSurface
----------------
...
function hit(a: Renderable | Renderable[], b: Renderable | Renderable[],  react = false, bounce = false, global: boolean, extra?: (...args: any[]) => void) {
  ...
  function rectangleVsRectangle(a: Rectangle, b: Rectangle) {
  ...
  function circleVsRectangle(a: Circle, b: Rectangle) {
    //If the rectangles shouldn't react to the collision, just
    //test to see if they're touching
    if (!react) {
